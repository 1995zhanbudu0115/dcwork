...
def fetch_trd_features():
    #交易数据
    trd_data = pd.read_csv(train_path+'/train_trd.csv')
    ...
    trd_features.to_csv(train_path+'/trd_features.csv',index=False)
    return trd_features
    
def fetch_beh_features():
    #用户app行为数据
    beh_data = pd.read_csv(train_path+'/train_beh.csv')
    beh_data['date'] = beh_data['page_tm'].str.split(' ').str.get(0)
    # 近30天每个用户访问页面的总次数
    beh_times_page = beh_data.groupby(['id','page_no'])['flag'].count().unstack().fillna(0)
    beh_times_page = beh_times_page.add_prefix('beh_times_page_')
    ben_times_app = beh_data.groupby(['id'])['flag'].count()
    # 近30天每个用户访问app的日均次数
    ...
    beh_features.to_csv(train_path+'/beh_features.csv',index=False)
    
def fetch_tag_features():
    #用户标签数据
    #tag_data = pd.read_csv('./data/train/train_tag.csv')
    tag_tag_data = pd.read_csv(train_path+'/train_tag.csv')
#    查看性别数据，发现有缺失，但缺失数据比例小于百分之2，
#    所以性别缺失数据可以剔除
    tag_data = tag_data[tag_data['gdr_cd'] != r'\N']
    ...
    tag_data = tag_data[tag_data['mrg_situ_cd'] != '~']
    ...
    tag_data.drop(drop_index,axis=1,inplace=False)
    tag_data = pd.concat([tag_data,tag_features],axis=1)
    tag_data.to_csv(train_path+'/tag_features.csv',index=True)
    return tag_data
    
def combine_features():
    if os.path.exists(train_path+'/beh_features.csv'):
        beh_features = pd.read_csv(train_path+'/beh_features.csv')
    else:
        beh_features = fetch_beh_features()
        
    if os.path.exists(train_path+'/trd_features.csv'):
        trd_features = pd.read_csv(train_path+'/trd_features.csv')
    else:
        trd_features = fetch_trd_features()
        
    if os.path.exists(train_path+'/tag_features.csv'):
        tag_features = pd.read_csv(train_path+'/tag_features.csv')
    else:
        tag_features = fetch_tag_features()
    features = tag_features.merge(trd_features,how='left')
    features = features.merge(beh_features,how='left')
    features = features.fillna(0)
    features.to_csv(train_path+'/features.csv',index=False)
    return features
    
def select_features():
    if os.path.exists(train_path+'/features.csv'):
        features = pd.read_csv(train_path+'/features.csv')
    else:
