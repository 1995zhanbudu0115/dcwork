...
class TradaBoostClassifier():
    ...
    def fit(self, trans_T, trans_S, label_T, label_S, test):
        ...
        # 初始化权重
        weight_source = np.ones([self.row_S, 1]) / self.row_S
        weight_target = np.ones([self.row_T, 1]) / self.row_T
        ...
        for i in range(self.epoch):
            ...
            print("Error Rate in target data: ", error_rate, 'round:', i, 'all_round:', self.epoch)                               
            ...
            for j in range(self.row_T):
                weights[self.row_S + j] = weights[self.row_S + j] * np.power(self.beta_Tree[0, i],(-np.abs(result[self.row_S + j] - label_T[j])))
            # 调整辅域样本权重
